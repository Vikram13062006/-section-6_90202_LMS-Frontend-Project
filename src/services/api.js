...
// ✅ ONLY ONE BACKEND URL (NO CONFUSION)
const API_BASE_URL =
...
export const extractApiErrorMessage = (
  ...
  if (typeof data === "string") {
    ...
    if (looksLikeHtml && import.meta.env.PROD) {
      return "Backend API route is not configured properly. Check backend URL.";
    }
  }

  if ((status === 404 || status === 502 || status === 503) && import.meta.env.PROD) {
    return "Backend API is unavailable. Check backend deployment.";
  }
...
export const assignmentApi = {
  ...
  grade: (submissionId, payload) =>
    api.post(`/assignments/submissions/${submissionId}/grade`, payload),
};
...
export const quizApi = {
  ...
  submit: (id, studentId, payload) =>
    api.post(`/quizzes/${id}/submit?studentId=${studentId}`, payload),
};

export const progressApi = {
  save: (payload) => api.post("/student/progress", payload),
  getByStudent: (studentId) =>
    api.get(`/student/progress?studentId=${studentId}`),
  getSingle: (studentId, courseId) =>
    api.get(
      `/student/progress/course?studentId=${studentId}&courseId=${courseId}`
    ),
};
